...
# ---------------------------------------------------------------------------
# dna_workflows_scan MCP thin-shell test
# ---------------------------------------------------------------------------
...
def test_dna_workflows_scan_forwards_to_service(monkeypatch):
    import services
    import mcp_server.tools.dna as dna_tool

    rec = _Recorder(return_value=[{"module_path": "src/foo"}])
    monkeypatch.setattr(services, "scan_workflows", rec)

    fmcp = _FakeMCP()
    dna_tool.register(fmcp)
    out = fmcp.tools["dna_workflows_scan"](
        module_paths=["src/foo"],
        keywords=["target"],
        cwd="/proj",
    )
    ...
    assert out == [{"module_path": "src/foo"}]
