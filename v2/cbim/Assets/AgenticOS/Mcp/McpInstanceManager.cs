...
namespace CBIM.Mcp
{
    ...
    ///
    /// 与 <see cref="McpManager"/>（基于 <see cref="McpRefToken"/> 引用计数）的职责区别：
    ///   - McpManager：精细引用计数，适合多 owner 共享 + 零引用自动释放的生命周期场景。
    ///   - McpInstanceManager：面向「脑区」brainId 键，显式 GetOrCreate / Release，
    ///     按 <see cref="McpDescriptor.Shared"/> 切换全局共享 vs 隔离实例两种策略。
    ///     适合 AgenticOS 装配侧：一个 brain 用一个 brainId，引用集合精确追踪共享实例归属。
    ///
    /// 两种模式：
    ///   Shared = true  → _sharedInstances 键为 mcpId（全局唯一实例），
    ///                     _sharedRefs[mcpId] 记录持有该实例的 brainId 集合；
    ///                     所有引用归零时才 Dispose 实例。
    ///   Shared = false → _isolatedInstances 键为 (mcpId, brainId)（每个脑区独占独立实例）。
    ///
    /// 并发安全：单锁 <c>_gate</c> 保护所有字典访问；Dispose 在锁外执行（避免阻塞）。
    ///
    /// 生命周期：
    ///   - <see cref="GetOrCreate"/> 首次为某个 key 调用时启动实例，后续返回缓存。
    ///   - <see cref="Release"/> 移除对应引用，引用归零时 Dispose 实例。
    ///   - <see cref="Dispose"/> 关闭所有仍活跃的实例（连接 / 子进程）。
    ///
    /// MCP SDK 情况：
    ///   本模块不直接引入 Microsoft.Agents.AI.Mcp NuGet（铁律：基建层零外部依赖）。
    ///   实例创建通过注入的 <see cref="IMcpClientStarter"/> SPI 委托给装配侧——
    ///   装配侧持有 SDK 依赖，实现 Start 内部调 McpClientFactory.CreateStdio / CreateSse。
    ///   若 starter 未注入或未配置，Start 应抛 <see cref="NotImplementedException"/>，
    ///   并附带 "需要 ModelContextProtocol NuGet 包 (Microsoft.Agents.AI.Mcp)" 提示。
    /// </summary>
    public sealed class McpInstanceManager : IDisposable
    {
        ...
        /// 不能为 null。
        /// </param>
        public McpInstanceManager(IMcpClientStarter starter)
        {
...
#region 内部辅助
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(McpInstanceManager));
        }
