...
#region Files 工具
        ...
        {
            if (storage == null) throw new ArgumentNullException(nameof(storage), "Files family requires a FileBackend storage instance");
            return AIFunctionFactory.Create(
                ...
                (Func<string, string, string>)((pattern, root) =>
                {
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < sorted.Count; i++)
                        {
                            ...
                            if (sb.Length > 0 && byteBudget - approxBytes < 0) { capped = true; break; }
        ...
        private static AIFunction BuildRunCommand(ToolSandbox sandbox)
        {
            return AIFunctionFactory.Create(
                (Func<string, string, int, string>)((command, workDir, timeoutMs) =>
                {
                    ...
                    if (workDir != null)
                    {
                        try { resolvedWorkDir = PathGuard.Normalize(workDir, sandbox); }
...
#region Files 私有辅助
        ...
        private static string TruncateByLines(string text, int offset, int maxLines, long maxResultBytes)
        {
            ...
            for (int i = start; i < end; i++)
            {
                ...
                if (sb.Length > 0 && byteBudget - approxBytes < 0) { byteTruncated = true; break; }
            ...
            if (lineTruncated || byteTruncated)
            {
                ...
                if (byteTruncated) sb.Append(", byte cap reached");
                sb.Append("]");
            }
            return sb.ToString();
        }
        ...
        {
            if (sandbox.BlockedExtensions == null || sandbox.BlockedExtensions.Count == 0) return false;
...
#region Search 私有辅助
            ...
            ref long byteBudget)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                for (int j = ctxStart; j < i; j++)
                {
                    ...
                    if (!AppendLine(sb, file + "-" + (j + 1) + "- " + lines[j], ref byteBudget)) { capped = true; return; }
                    lastEmittedLine = j;
                }
                if (!AppendLine(sb, file + ":" + (i + 1) + ": " + lines[i], ref byteBudget)) { capped = true; return; }
                lastEmittedLine = i;
                ...
                matches++;
                if (maxMatches > 0 && matches >= maxMatches) { capped = true; return; }
            }
        }
        ...
        private static bool AppendLine(StringBuilder sb, string line, ref long byteBudget)
        {
            ...
            if (sb.Length > 0 && byteBudget - approxBytes < 0) return false;
...
#region Bash 私有辅助
        ...
        {
            if (string.IsNullOrEmpty(text)) { usedBytes = 0; return text ?? string.Empty; }
            if (budgetBytes <= 0)           { usedBytes = 0; return "[truncated]"; }

            int byteCount = Encoding.UTF8.GetByteCount(text);
            if (byteCount <= budgetBytes)   { usedBytes = byteCount; return text; }

            const string suffix = "[truncated]";
            int suffixBytes = Encoding.UTF8.GetByteCount(suffix);
            long allowedForContent = budgetBytes - suffixBytes;
            if (allowedForContent <= 0)     { usedBytes = suffixBytes; return suffix; }
