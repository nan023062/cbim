...
namespace CBIM.AgentSystem.Kernel.Synapse.Orchestrator
{
    ...
    ///     <see cref="ExecutorFailedEvent"/> 直接转 <see cref="NeuronOutput.IsError"/>=true，
    ...
    public sealed class CBIMOrchestrator
    {
        ...
        ///   <item>循环结束（halt 或 stream 完成）后据 errors / finalSummary 产 <see cref="NeuronOutput"/>。</item>
        /// </list>
        ///
        /// <para>失败转译：</para>
        /// <list type="bullet">
        ///   <item>循环中收到任何错误事件 → <see cref="NeuronOutput.IsError"/>=true，
        ///     <see cref="NeuronOutput.ErrorMessage"/> 由收到的错误消息以 <c>"; "</c> 拼接。</item>
        ///   <item>halt 但 finalSummary 仍为 null（无 ReturnNode 触发输出）→
        ///     <see cref="NeuronOutput.IsError"/>=true，错误描述固定字符串。</item>
        ...
        public async Task<NeuronOutput> RunAsync(
            ...
            CancellationToken ct)
        {
            ...
            var startMessage = new CircuitMessage(
                ...
                history: new Dictionary<string, NeuronOutput>(StringComparer.Ordinal));
